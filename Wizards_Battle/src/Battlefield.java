...
public class Battlefield extends JPanel implements ActionListener {
    ...
    public Battlefield(Start temp, String name, String c1, String c2) {
        ...
        ImageIcon i = new ImageIcon(Objects.requireNonNull(getClass().getResource("map/" + name + ".png")));
        img = i.getImage();
        p1 = new Player1(c1);
        //p2 = new Player2(c2);
      ...
      for (int j = 0; j < 18; j++) {
            HpBarP1[j] = new ImageIcon(Objects.requireNonNull(getClass().getResource("HealthBar/p1/HP" + (j + 1) + ".png")));
         //   HpBarP2[j] = new ImageIcon(Objects.requireNonNull(getClass().getResource("HealthBar/p2/HP" + (j + 1) + ".png")));
        }
        for (int k = 0; k < 3; k++) {
            flyL[k] = new ImageIcon(Objects.requireNonNull(getClass().getResource("HAPPY/l/HAPPYtalk" + (1 + k) + ".png")));
            flyR[k] = new ImageIcon(Objects.requireNonNull(getClass().getResource("HAPPY/r/HAPPYtalk" + (1 + k) + ".png")));
        }
    ...
    public void check() {
        ...
        // cheer();
        if (rp1.intersects(rp2) && p1.attach && !p2.defense && !p2.die) {
            p2.setHP(p2.getHP() - 2);
            p2.damge = true;
            p2.Damege();
            HPBAR2();
        ...
        if (rp2.intersects(rp1) && p2.attach && !p1.defense && !p1.die) {
            p1.setHP(p1.getHP() - 2);
            p1.damge = true;
            p1.Damege();
            HPBAR1();
        ...
        batteltime = new Timer(1000, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (sec == 0 && min == 0) {
                    ...
                    if (p1.getHP() > p2.getHP()) {
                        JOptionPane.showMessageDialog(null, "Player 1 is the winer", "Time's up",
                    ...
                    if (p2.getHP() > p1.getHP()) {
                        JOptionPane.showMessageDialog(null, "Player 2 is the winer", "Time's up",
                                JOptionPane.INFORMATION_MESSAGE);
                        p2.setWin(true);
                        flying.stop();
